...
"""
    ...
    train_jsonl_path = f"agent-debates/train-agent-debate-first-part-{start_index}-{end_index}.jsonl"
    ...
    debate_json_path = f"agent-debates/train-agent-debate-challenge{MODEL_NAME_surfix}-part-{start_index}-{end_index}.jsonl"
    ...
    with open(train_jsonl_path, "r", encoding="utf-8") as r_f:
        ...
        for i, line in enumerate(r_f):
            ...
            dataset, type, question, answer, agent_1_debate = data["dataset"], data["type"], data["question"], data["answer"], data["agent_1_debate"]

            agent_1_argument = clean_json_string(agent_1_debate)
            agent_1_argument = remove_consecutive_repeated_sentences(agent_1_argument)

            PROMPTED_QUERY = f"Role: You are a critical thinker, debater and medical knowledge analyst Bot.\n\n\
                Base on the Q&A and the Original Analysis: \nQuestion: {question}\nAnswer: {answer}\nOriginal Analysis: {agent_1_argument}\n\n\
                Your task is to criticize the Original Analysis by demonstrating its insufficiency or inadequacy, then provide an enhanced analysis that addresses the limitations and explores additional dimensions. Note that: empty analysis (e.g. []) or replication in the JSON's inner content trigger insufficiency flags.\n\
            ...
            if len(INPUT_BATCH) == batch_size:
                ...
                for decoded_ans, data_dict in zip(DECODED_ANSWERS, DICT_BATCH):
                    ...
                    with open(debate_json_path, "a", encoding="utf-8") as f:
                        ...
                        temp_dict[f"agent_2_challenge"] = decoded_ans.strip()
